...
//Below this code I just copy paste from Stack Overflow and slightly modify it and I also mansion on MainActivity that how we can access user location
//I comment it on below on MainActivity
...
@Composable
fun ExampleForegroundLocationTrackerScreen(latitudeAndLongitude: (Double,Double) -> Unit) {

    ForegroundLocationTracker(snackbarHostState = SnackbarHostState(),
        onLocationUpdates = {
            Log.i("LogTag", "Current location is: $it")
            latitudeAndLongitude(it.latitude,it.longitude)
        })
}